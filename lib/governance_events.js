...
async function treatResponseFromGovernanceAA(objResponse, asset, main_aa) {
    ...
    if (data.name) {
        ...
        const city_label = data.name === "mayor" ? `|${data.city ?? 'city'}` : ''; // only for mayor 

        if (data.commit) {
            event.type = "commit";
            event.value = await DAG.readAAStateVar(governanceAAAddress, data.name);
        } else {
            event.leader_value = await DAG.readAAStateVar(governanceAAAddress, 'leader_' + data.name + city_label);
            event.leader_support = await DAG.readAAStateVar(governanceAAAddress, 'support_' + data.name + city_label + '_' + event.leader_value)
            if (data.value === undefined) {
                event.type = "removed_support";
            } else {
                event.type = "added_support";
                event.value = data.value;
                event.added_support = await DAG.readAAStateVar(main_aa, 'user_land_city_' + objResponse.trigger_address);
                event.support = await DAG.readAAStateVar(governanceAAAddress, 'support_' + data.name + city_label + '_' + getValueKey(data.value));
            }
        }
    }
